...
export const DEFAULT_CREATIVITY = 0.5;
export const MIN_CREATIVITY = 0;
export const MAX_CREATIVITY = 1;

export const EXTRACTOR_SYSTEM_PROMPT = `You extract story-generation parameters from free-form user text.
Respond with ONLY valid JSON (no markdown fences, no commentary) using this shape:
{
  "subject": string | null,  
...
- creativity: model temperature from 0 (predictable) to 1 (very creative). Map "low"→0.1, "medium"/"med"→0.5, "high"→0.9. null if not mentioned.
- mood: "happy" or "scary". null if not mentioned.
- lines: story length as integer 1–${MAX_STORY_LINES}. null if not mentioned.
- title: optional story title. null if not mentioned.`;
...
export function normalizeCreativity(creativity) {
  if (creativity === undefined || creativity === null || creativity === "") {
    return DEFAULT_CREATIVITY;
  }
  const n = Number(creativity);
  if (!Number.isFinite(n) || n < MIN_CREATIVITY || n > MAX_CREATIVITY) {
    throw new Error(
      `Creativity must be a number from ${MIN_CREATIVITY} to ${MAX_CREATIVITY}.`,
    );
  }
  return n;
}
