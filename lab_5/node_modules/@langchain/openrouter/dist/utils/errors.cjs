...
var OpenRouterError = class OpenRouterError extends ns.brand(_langchain_core_errors.LangChainError) {
	...
	*/
	static async fromResponse(response) {
		...
		if (response.status === 401 || response.status === 403) return new OpenRouterAuthError(message, code, error?.metadata);
		if (response.status === 429) return new OpenRouterRateLimitError(message, code, error?.metadata);
