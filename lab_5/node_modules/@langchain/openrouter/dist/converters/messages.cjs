...
function convertOpenRouterResponseToBaseMessage(choice, rawResponse) {
	...
	if (typeof assistantMessage.reasoning === "string" && assistantMessage.reasoning.length > 0) message.additional_kwargs.reasoning_content = assistantMessage.reasoning;
	if (Array.isArray(assistantMessage.reasoning_details) && assistantMessage.reasoning_details.length > 0) message.additional_kwargs.reasoning_details = assistantMessage.reasoning_details;
...
function convertOpenRouterDeltaToBaseMessageChunk(delta, rawChunk, defaultRole) {
	...
	if (typeof delta.reasoning === "string" && delta.reasoning.length > 0) chunk.additional_kwargs.reasoning_content = delta.reasoning;
	if (Array.isArray(delta.reasoning_details) && delta.reasoning_details.length > 0) chunk.additional_kwargs.reasoning_details = delta.reasoning_details;
